...
void V2::World::importDefaultPops()
{
	...
	auto filenames = Utils::GetAllFilesInFolder(popsFolder);
...
void V2::World::copyModFiles() const
{
	if (!theConfiguration.getVic2ModName().empty())
	{
		...
		//	/gfx/interface
		const auto& leaders = Utils::GetAllFilesInFolder(mod + "/gfx/interface/leaders");
		for (const auto& leader: leaders)
		{
			fs::copy_file(mod + "/gfx/interface/leaders/" + leader, output + "/gfx/interface/leaders/" + leader, fs::copy_options::recursive);
		}
		const auto& gfxInterfaceFiles = Utils::GetAllFilesInFolder(mod + "/gfx/interface");
		...
		//	/interface
		const auto& interfaceFiles = Utils::GetAllFilesInFolder(mod + "/interface");
...
void V2::World::convertEvents()
{
	...
	if (Utils::DoesFolderExist("blankMod/output/events"))
	{
		evtFiles = Utils::GetAllFilesInFolder("blankMod/output/events");
	}
...
std::shared_ptr<V2::State> V2::World::getStateByID(int stateID) const
{
	/*for (const auto& country: countries)
	{
		const auto& states = country.second->getStates();
		auto stateItr = std::find_if(states.begin(), states.end(), [stateID](const auto& state) {
			return (state.getStateID() == stateID);
		});
...
int V2::World::getActualStateID(int provID, const std::map<int, std::set<int>>& theMap) const
{
	for (const auto& mapping: theMap)
	{
		for (const auto& province: mapping.second)
		{
			if (province == provID)
